% Exponential growth is simulated. We calculate the binned data.

% clear
% close

function [el_rate_l_f_bin, el_rate_f_bin, a_f_bin] = birth_to_div_gr_v_age_exp_sims(num_in)

[num,txt,raw] = xlsread('params.xlsx');

% Calculate cell cycle characteristics
tau=num(num_in,1); % generation time in minutes
gr = num(num_in,14); % mean growth rate
...
%-----Noise parameters
cvl = 0;%num(num_in,15)*num(num_in,14); % std dev of growth rate
cvr= num(num_in, 17); % std dev of division noise
cve = num(num_in, 16); % standard deviation (std dev) of meausrement error
