% call function to call various files and plot growth rate vs age info as
% shown in the main text of the paper.

% To do: Choose files to execute (file_call variable)

clear

%-------Give your choice
file_call= 1; % 1 to call all data of L vs t, 2 to call data in 06_24, 3
% to call 09_03 and 4 to call 09_03_2
%-------

if file_call == 2
    [vs, ts, ty] = analysis_06_24();
elseif file_call==3
    [vs, ts, ty] = analysis_09_03();
elseif file_call==4
    [vs, ts, ty] = analysis_09_03_2();  
elseif file_call==1
    [vs_1, ts_1, ty_1] = analysis_06_24();
    [vs_2, ts_2, ty_2] = analysis_09_03();
    [vs_3, ts_3, ty_3] = analysis_09_03_2();
    vs = [vs_1 vs_2 vs_3];
    ts = [ts_1 ts_2 ts_3];
    ty = [ty_1 ty_2 ty_3];   
end
...
% collect simulation results for linear growth to make plots for comparing 
% against experiments

repeat = 500;
el_rate_f_sim = NaN(repeat,30); el_rate_l_f_sim =NaN(repeat,30); a_f_sim=NaN(repeat,30);
for i=1:repeat
    [el_rate_l_f_sim_e, el_rate_f_sim_e, a_f_sim_e] = birth_to_div_gr_v_age_sims(file_call);
...
% collect simulation results for exponential growth to make plots for 
% comparing against experiments

repeat = 500;
el_rate_f_exp_sim = NaN(repeat,30); el_rate_l_f_exp_sim =NaN(repeat,30); a_f_exp_sim=NaN(repeat,30);
for i=1:repeat
    [el_rate_l_f_exp_sim_e, el_rate_f_exp_sim_e, a_f_exp_sim_e] = birth_to_div_gr_v_age_exp_sims(file_call);
    el_rate_l_f_exp_sim(i,:)=el_rate_l_f_exp_sim_e;
    el_rate_f_exp_sim(i,:)=el_rate_f_exp_sim_e;
    a_f_exp_sim(i,:) = a_f_exp_sim_e;
end

el_rate_f_exp_sim_f = mean(el_rate_f_exp_sim); el_rate_l_f_exp_sim_f = mean(el_rate_l_f_exp_sim); a_f_exp_sim_f = mean(a_f_exp_sim);
...
s1=plot(a_f_sim_f,el_rate_l_f_sim_f, 'LineWidth',4, 'Color','r');
s1=plot(a_f_exp_sim_f,el_rate_l_f_exp_sim_f, 'LineWidth',4, 'Color','b');
...
s.MarkerEdgeColor='k';
s.MarkerFaceColor=[0.85, 0.33, 0.1];
s1=plot(a_f_sim_f,el_rate_f_sim_f, 'LineWidth',4, 'Color','r');
s1=plot(a_f_exp_sim_f,el_rate_f_exp_sim_f, 'LineWidth',4, 'Color','b');
eb = errorbar(bin(ind),da(ind),err(ind), 'LineStyle', 'none', 'LineWidth', 1.5, 'Color', [0.85, 0.33, 0.1]);
set(get(get(eb,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
ylabel('dL/dt (\mum/hr)');
xlabel('Age');
