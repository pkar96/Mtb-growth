...
%-----Noise parameters
cvl = num(num_in, 13)*num(num_in, 12); % std linear growth rate
cvr= num(num_in, 17); % std dev of division noise
cve= num(num_in, 16);   % std dev of experimental noise
cvt2= sqrt(((num(num_in, 2)*num(num_in, 3))^2-cve^2)*alpha*(2-alpha)-4*cvr^2*num(num_in,2)^2)*2; % std dev of size additive division noise
